import { motion } from 'framer-motion';

const SafetySecurity = () => {
  const measures = [
    {
      title: '24/7 Security',
      description: 'Round-the-clock security personnel and surveillance systems.',
      icon: '🔒',
    },
    {
      title: 'Emergency Response',
      description: 'Quick response protocols and trained emergency response team.',
      icon: '🚨',
    },
    {
      title: 'Health & Wellness',
      description: 'On-site medical facilities and regular health check-ups.',
      icon: '🏥',
    },
  ];

  const protocols = [
    {
      name: 'Access Control',
      description: 'Strict entry and exit procedures with ID verification',
    },
    {
      name: 'CCTV Surveillance',
      description: 'Comprehensive camera coverage throughout the campus',
    },
    {
      name: 'Fire Safety',
      description: 'Regular fire drills and modern fire safety equipment',
    },
    {
      name: 'Health Protocols',
      description: 'Regular health screenings and hygiene maintenance',
    },
  ];

  return (
    <div className="max-w-7xl mx-auto px-4 py-12 translate-y-10">
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.5 }}
        className="text-center mb-16"
      >
        <h1 className="text-4xl font-bold text-gray-900 mb-4">Safety & Security</h1>
        <p className="text-xl text-gray-600">
          Ensuring a safe and secure learning environment
        </p>
      </motion.div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-16">
        <motion.div
          initial={{ opacity: 0, x: -20 }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ duration: 0.5, delay: 0.2 }}
          className="bg-white rounded-lg shadow-lg p-8"
        >
          <h2 className="text-2xl font-semibold text-gray-900 mb-6">Our Commitment</h2>
          <p className="text-gray-600 mb-6">
            The safety and security of our students and staff is our top priority.
            We maintain the highest standards of safety protocols and continuously
            update our security measures to ensure a protected learning environment.
          </p>
          <div className="space-y-4">
            <div className="flex items-center">
              <span className="text-blue-600 mr-2">✓</span>
              Trained security personnel
            </div>
            <div className="flex items-center">
              <span className="text-blue-600 mr-2">✓</span>
              Modern security systems
            </div>
            <div className="flex items-center">
              <span className="text-blue-600 mr-2">✓</span>
              Regular safety audits
            </div>
          </div>
        </motion.div>

        <motion.div
          initial={{ opacity: 0, x: 20 }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ duration: 0.5, delay: 0.4 }}
          className="bg-white rounded-lg shadow-lg p-8"
        >
          <h2 className="text-2xl font-semibold text-gray-900 mb-6">Security Protocols</h2>
          <div className="grid grid-cols-1 gap-4">
            {protocols.map((protocol, index) => (
              <motion.div
                key={protocol.name}
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.3, delay: index * 0.1 }}
                className="bg-gray-50 rounded-lg p-4"
              >
                <h3 className="text-lg font-medium text-gray-900 mb-2">{protocol.name}</h3>
                <p className="text-gray-600">{protocol.description}</p>
              </motion.div>
            ))}
          </div>
        </motion.div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
        {measures.map((measure, index) => (
          <motion.div
            key={measure.title}
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5, delay: 0.2 * index }}
            className="bg-white rounded-lg shadow-lg p-6 text-center"
          >
            <div className="text-4xl mb-4">{measure.icon}</div>
            <h3 className="text-xl font-semibold text-gray-900 mb-3">{measure.title}</h3>
            <p className="text-gray-600">{measure.description}</p>
          </motion.div>
        ))}
      </div>

      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.5, delay: 0.8 }}
        className="text-center"
      >
        <button className="bg-blue-600 text-white px-8 py-3 rounded-lg hover:bg-blue-700 transition-colors duration-300">
          Learn More About Safety Measures
        </button>
      </motion.div>
    </div>
  );
};

export default SafetySecurity; 